...
Стоит отдельно отметить, что `UserDefaults` позволяет хранить данные **только на конкретном устройстве пользователя**, а не на всём его аккаунте, как вы могли бы подумать.
...
Для подобной задачи необходимо использовать **сервер** в качестве единого источника информации. Хранилище на устройстве (`UserDefaults`) никак не синхронизируется между девайсами автоматически.
...
Данные не должны синхронизироваться между устройствами: даже если пользователь установил приложение на `iPad` после `iPhone`, логично показать ему онбординг заново.

## Что такое `UserDefaults` и `AppStorage`

### `UserDefaults` 
Система хранения пар "ключ-значение". Хранит простые типы: `Bool`, `Int`, `String`, `Array`, `Dictionary`.
...
```
### `AppStorage` (современный подход)
Property Wrapper от `SwiftUI`. Позволяет привязать переменную напрямую к `UserDefaults` и **автоматически обновляет интерфейс** при изменении значения.
...
**Используйте UserDefaults, если:**
- Нужно удалить конкретный ключ (`removeObject`).
- Логика хранения сложнее, чем просто чтение/запись.

**Используйте AppStorage, если:**
- Вы пишете на SwiftUI.
- Нужно, чтобы интерфейс реагировал на изменение настройки.
- Вы храните примитивные типы (`Bool`, `Int`, `String`).

## Лучшие практики

**1. Не храните большие данные**
`UserDefaults` не предназначен для изображений, больших `JSON` или списков из тысяч элементов. Используйте `FileManager` или `CoreData`

**2. Избегайте "магических строк"**
Плохо, так как если вдруг вам придётся изменять ключ (`usernmae`), а рано или поздно – придётся так как в нём есть ошибка – вам потребуется изменить его во всех места, где вы на него ориентируете и вы непременно ошибётесь.
```swift
...
@AppStorage(Keys.username) var username: String = ""
```

Сейчас вы указали ключ – в одном месте и если вам потребуется его изменить – это не проблема, достаточно пометь `static let username`
...
- `UserDefaults` / `@AppStorage` — для **локальных** мелочей на одном устройстве.
- **Сервер** — для данных, которые должны быть едины на всех устройствах пользователя (корзина, подписки, статус заказа).
- **Идеальный кейс:** хранение флага первого запуска, темы оформления, последнего `выбранного ID`.

Если вы помните, что данные не синхронизируются между устройствами, и используете `UserDefaults` только для того, что действительно должно жить только на этом конкретном `iPhone` — вы на верном пути.

## Что такое `UserDefaults`?

`UserDefaults` — это система хранения пар "ключ-значение" (Key-Value). Она идеально подходит для:
- Настроек приложения
- Сохранения состояния (например, "авторизован ли пользователь")
- Простых типов данных (`Integer`, `Bool`, `String`, `Array`, `Dictionary`).

Она работает как легковесная база данных, которая живёт в песочнице вашего приложения.

## Что такое AppStorage?

`AppStorage` — это `Property Wrapper` (обёртка свойств), которую `Apple` представила в `SwiftUI`. Она буквально "склеивает" переменную в коде с записью в `UserDefaults`.

Главный плюс: **Автоматическое обновление UI**. Когда вы меняете значение переменной, экран перерисовывается сам.

### Базовый синтаксис

Вы просто объявляете переменную в теле View:
...
Важно понимать: `AppStorage` не создаёт новое хранилище. Это просто "надстройка" над `UserDefaults`. Все сохранённые данные вы можете найти в стандартном домене `UserDefaults` вашего приложения.
...
Здесь нет лишнего кода. При нажатии на кнопку "Войти" переменная становится `true`, и `SwiftUI` моментально переключает интерфейс на приветствие.
...
```
Даже если вы закроете и перезапустите приложение, счётчик сохранит последнее значение. Это происходит потому, что `AppStorage` синхронизируется с диском при изменении данных.
...
```
Размер данных. Даже с `Codable`, `UserDefaults` не предназначен для хранения больших объёмов данных. Если структура содержит много полей или вы храните сотни объектов — рассмотрите `SwiftData` или `FileManager`.
