...
Стоит отдельно отметить, что `UserDefaults`  позволяет хранить данные **только на конкретном устройстве пользователя**, а не на всём его аккаунте, как вы могли ошибочно бы подумать.
...
`UserDefaults` очень похож на словарь. **В нём как и в словаре ключевую роль играет ключ – значение, по которому мы сохраняем данные, а в последующем можем поулчить.**

**Сохранение:**
```swift

/// Стандартное хранилище, в подавляющем большинстве случаев – именно его и нужно использовать.
...
`@AppStorage` используется во `View`, что бы было удобнее хранить примитивные типы данных, например `String` для Поля ввода.
...
`UserDefaults` это и есть самая основа. Дело в том, что в `AppStorage` мы можем сохранить лишь примитивные значения, к тому же тоьлко те, которые показаны во `View` и с которыми взаимодействует пользователь.

Но есть много сулчаев, когда нам нужно хранить сложные данные (Созданные нами структуры например) и которые вовсе не представлены во `View` – в таком случае нам на помощь приходит `UserDefaults`

`UserDefaults` это супер просто. Выше мы уже разбирали пример работы, но давайте повторим – 
...
Что значит `.standard` ? – Это стандартное хранилище, о котором за нас уже позаботились в `Apple`, просто используйте его, когда перед вами встанет задача создать своё собсвтенное – вы сделаете это самостоятельно.
...
`UserDefaults` не предназначен для изображений, больших `JSON` или списков из тысяч элементов. Используйте `FileManager` или `SwiftData`
...
@AppStorage(UserDefaultKeys.username) var username: String = ""
```
...
Помимо этого `UserDefaultKeys.username` привносит гоорааздо больше информации, чем просто "username" – мы сразу понимаем, что речь идёт о `UserDefault`
...
```
**Даже если вы закроете и перезапустите приложение, счётчик сохранит последнее значение.** Это происходит потому, что `AppStorage` синхронизируется с дискетой при изменении данных.
...
Не стоит забывать, что после того как вы получили данные из `UserDefaults` и обновили значения – нужно обязательно вновь сохранить эти данные, иначе в хранилище `UserDefaults` так и останутся старые данные.
